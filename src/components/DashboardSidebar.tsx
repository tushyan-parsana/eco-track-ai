 ...
 * @description Server component rendering the fixed sidebar navigation
 * for the dashboard section with navigation links and the user's eco score.
 */
...
export default async function DashboardSidebar() {
  ...
  return (
    <aside
      className="w-64 fixed left-0 top-16 bottom-0 glass-dark border-r border-white/10 p-4 flex flex-col gap-2 z-40"
      aria-label="Dashboard navigation"
    >
      <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4 px-3 mt-4">
        Menu
      </div>
      <nav className="flex-1 flex flex-col gap-1" aria-label="Dashboard menu">
        {navItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
