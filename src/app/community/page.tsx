      ...
      <div className="container mx-auto max-w-5xl">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-slate-600 dark:text-slate-400">
            See how you stack up against other EcoTrack users and view our
            collective global impact.
          </p>
        </div>

        <div className="grid md:grid-cols-3 gap-6 mb-12" role="group" aria-label="Community statistics">
          <GlassCard className="text-center">
            <Users className="w-8 h-8 mx-auto text-brand-500 mb-3" aria-hidden="true" />
          ...
          <GlassCard className="text-center">
            <Globe2 className="w-8 h-8 mx-auto text-blue-500 mb-3" aria-hidden="true" />
            <h3 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">
              540k
            </h3>
          ...
          <GlassCard className="text-center">
            <TrendingDown className="w-8 h-8 mx-auto text-success-500 mb-3" aria-hidden="true" />
            <h3 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">
              2.4M
            </h3>
        ...
        <GlassCard>
          ...
          {leaderboard.length > 0 ? (
            <ol className="space-y-4" aria-label="Community leaderboard rankings">
              {leaderboard.map((user) => (
                <li
                  ...
                  aria-label={`Rank ${user.rank}: ${user.name}${user.isUser ? " (You)" : ""}, Eco Score ${user.score}`}
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-8 font-bold text-center ${
                        user.rank <= 3 ? "text-brand-500" : "text-slate-400"
                      }`}
                      aria-hidden="true"
                    >
                      #{user.rank}
                    ...
                    <div
                      className="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-800 flex items-center justify-center font-semibold text-slate-600 dark:text-slate-300"
                      aria-hidden="true"
                    >
                      {user.avatar}
                    </div>
            ...
            <div className="text-center text-slate-500 py-8">
              No users found. Be the first to calculate your footprint!
            </div>
          )}
        </GlassCard>
      </div>
